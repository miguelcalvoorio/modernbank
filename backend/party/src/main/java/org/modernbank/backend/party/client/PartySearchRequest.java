...
public class PartySearchRequest {
    ...
    @Schema(
        required = false,
        example = "1970-01-01",
        description = "Performs exact match on the party's date of birth")
    private Date dateOfBirth;

    @Schema(
        required = false,
        example = "ES",
        description = "Performs exact match on the party's nationality")
    private TypeCountry nationality;

    @Schema(
        required = false,
        example = "ACTIVE",
        description = "Performs exact match on the party's status")
